...
export function Navbar() {
  ...
  return (
    <nav 
      className={`fixed top-0 z-50 w-full transition-all duration-300 ease-in-out transform ${
    ...
    >
      <div className="w-full max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 xl:px-8">
        <div className="flex items-center justify-between h-14 sm:h-16 lg:h-18">
          ...
          <Link 
            href="/" 
            className={`flex items-center gap-2 text-lg sm:text-xl lg:text-2xl font-bold tracking-tight transition-colors duration-200 flex-shrink-0 group ${
              isScrolled ? 'text-gray-900' : 'text-white'
            } hover:text-blue-500`}
          >
            {/* Fork and Spoon Logo */}
            <div className={`relative w-7 h-7 sm:w-8 sm:h-8 lg:w-9 lg:h-9 transition-colors duration-200 ${
              ...
              >
                ...
                <g transform="rotate(45 50 50)">
                  <rect x="47.5" y="20" width="5" height="60" rx="2.5" opacity="0.1" />
                </g>
              </svg>
            </div>
          ...
          <ul className="hidden xl:flex items-center gap-4 2xl:gap-8">
            {NAV_MENU.map(({ name, icon: Icon, href }) => (
              <NavItem key={name} href={href}>
                ...
                <span className="text-sm lg:text-base font-medium whitespace-nowrap">{name}</span>
              </NavItem>
            ))}
          </ul>

          {/* Desktop Auth - Simplified for medium screens */}
          <div className="hidden lg:flex items-center gap-2 xl:gap-4 flex-shrink-0">
            {!session ? (
              <div className="flex items-center gap-2">
                <button
                  onClick={() => signIn('google')}
                  className={`px-3 xl:px-4 py-2 text-xs xl:text-sm font-medium rounded-lg transition-all duration-200 whitespace-nowrap ${
                    isScrolled
                ...
                <Link
                  href="/admin/signin"
                  className={`px-3 xl:px-4 py-2 text-xs xl:text-sm font-medium rounded-lg transition-all duration-200 border whitespace-nowrap ${
                    isScrolled
              ...
              <div className="flex items-center gap-2 xl:gap-4 relative user-menu">
                ...
                >
                  ...
                  <ChevronDownIcon className={`w-4 h-4 text-gray-500 transition-transform duration-200 ${userMenuOpen ? 'rotate-180' : ''}`} />
                </button>
          ...
          <button 
            onClick={handleOpen} 
            className={`lg:hidden p-2 rounded-lg transition-all duration-200 touch-manipulation ${
              isScrolled 
          ...
          >
            <div className="relative w-5 h-5 sm:w-6 sm:h-6">
              <span className={`absolute top-1/2 left-1/2 w-5 sm:w-6 h-0.5 bg-current transform transition-all duration-300 ${
                open ? 'rotate-45 translate-x-0' : '-translate-x-2.5 sm:-translate-x-3 -translate-y-2'
              }`} />
              <span className={`absolute top-1/2 left-1/2 w-5 sm:w-6 h-0.5 bg-current transform transition-all duration-300 ${
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-sm sm:text-base font-medium text-gray-900 truncate">{session.user?.name}</p>
